...
class PlayState extends MusicBeatState
{
	...
	var dadStrum = new StrumLine();
	var playerStrum = new StrumLine();
	...
	override public function create()
	{
		...
		dadStrum.scrollFactor.set();
		playerStrum.scrollFactor.set();
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.scale.set(CoolUtil.lerp(iconP1.scale.x,1,0.2),CoolUtil.lerp(iconP1.scale.y,1,0.2));
		iconP2.scale.set(CoolUtil.lerp(iconP2.scale.x,1,0.2),CoolUtil.lerp(iconP2.scale.y,1,0.2));
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				daNote.y = (dadStrum.y - (Conductor.songPosition - daNote.strumTime) * (0.45 * FlxMath.roundDecimal(SONG.speed, 2)));

				// i am so fucking sorry for this if condition
				if (daNote.isSustainNote
					&& daNote.y + daNote.offset.y <= dadStrum.y + Note.swagWidth / 2
					&& (!daNote.mustPress || (daNote.wasGoodHit || (daNote.prevNote.wasGoodHit && !daNote.canBeHit))))
				{
					var swagRect = new FlxRect(0, dadStrum.y + Note.swagWidth / 2 - daNote.y, daNote.width * 2, daNote.height * 2);
	...
	private function handleInput(evt:KeyboardEvent):Void
	{
		...
		if (!paused && generatedMusic)
		{
			...
			playerStrum.members[data].animation.play('pressed');
			playerStrum.members[data].centerOffsets();
	...
	private function releaseInput(evt:KeyboardEvent):Void // handles releases
	{
		...
		keys[data] = false;
		playerStrum.members[data].animation.play("static");
		playerStrum.members[data].centerOffsets();
	}
